...
public final class GeneralPacketMojang : GeneralPacket {
    ...
    public init(bytes: [UInt8]) throws {
        ...
        self.length = length
        let (packetID, packet_read_bytes):(VariableIntegerJava, Int) = try bytes.readVarInt(byteOffset: length_read_bytes)
    ...
    public func readRemainingByteArray() throws -> [UInt8] {
        return try readByteArray(bytes: data.count - reading_index)
    }
    ...
    public func readByteArray(bytes: VariableIntegerJava) throws -> [UInt8] {
        return try readByteArray(bytes: bytes.value_int)
    }
    ...
    public func readStringArray(count: VariableIntegerJava) throws -> [String] {
        return try readStringArray(count: count.value_int)
    }
    ...
    public func readMap<T>(count: VariableIntegerJava, transform: () throws -> T) rethrows -> [T] {
        return try readMap(count: count.value_int, transform: transform)
    }
