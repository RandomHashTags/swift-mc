...
public struct SlotMojang: Codable, PacketEncodableMojangJava, PacketDecodableMojangJava {
    ...
    public func packetBytes() throws -> [UInt8] {
        ...
        if present {
            ...
            let itemID:VariableIntegerJava = try unwrapOptional(itemID, key_path: \Self.itemID, precondition: precondition)
            let itemCount:Int8 = try unwrapOptional(itemCount, key_path: \Self.itemCount, precondition: precondition)
